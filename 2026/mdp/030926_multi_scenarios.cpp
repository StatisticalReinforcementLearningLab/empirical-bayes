...
Vec4 zero_vec4() { return {0.0, 0.0, 0.0, 0.0}; }

Vec4 matvec(const Mat4 &A, const Vec4 &x) {
    Vec4 out = zero_vec4();
    for (int i = 0; i < 4; ++i) for (int j = 0; j < 4; ++j) out[i] += A[i][j] * x[j];
    return out;
}
...
Mat4 outer(const Vec4 &x) {
    Mat4 A = zero_mat4();
    for (int i = 0; i < 4; ++i) for (int j = 0; j < 4; ++j) A[i][j] = x[i] * x[j];
    return A;
}
...
Mat4 inverse4(const Mat4 &A) {
    ...
    for (int col = 0; col < 4; ++col) {
        int pivot = col;
        for (int r = col + 1; r < 4; ++r) if (std::fabs(aug[r][col]) > std::fabs(aug[pivot][col])) pivot = r;
        if (std::fabs(aug[pivot][col]) < 1e-12) throw std::runtime_error("Matrix inversion failed");
...
Vec4 mvn_sample(std::mt19937_64 &rng, const Vec4 &mu, const Mat4 &Sigma) {
    ...
    Vec4 out = mu;
    for (int i = 0; i < 4; ++i) for (int k = 0; k <= i; ++k) out[i] += L[i][k] * z[k];
    return out;
}
...
struct SimpleLinearMDP {
    ...
    SimpleLinearMDP(int n_, const Vec4 &mu_theta_, const Mat4 &Sigma_theta_, double alpha_, double beta_,
                    double gamma_, double sigma_s_, double sigma_r_, double mu_s0_, double sigma_s0_, uint64_t seed)
        : n(n_),
          mu_theta(mu_theta_),
          ...
          theta(n_),
          s(n_, 0.0) {
        for (int i = 0; i < n; ++i) theta[i] = mvn_sample(rng, mu_theta, Sigma_theta);
    }
    ...
    static Vec4 feature_vector(double s, int a) { return {1.0, s, static_cast<double>(a), s * static_cast<double>(a)}; }
...
Vec4 feature_vector(double s, int a) { return {1.0, s, static_cast<double>(a), s * static_cast<double>(a)}; }
...
struct UnpooledTS {
    ...
    UnpooledTS(int n_, double lambda_prior, uint64_t seed)
        : n(n_), rng(seed), Lambda(n_, identity_mat4(lambda_prior)), b(n_, zero_vec4()) {}
...
struct PooledTS {
    ...
    PooledTS(int n_, double lambda_prior, uint64_t seed)
        : n(n_), rng(seed), Lambda(identity_mat4(lambda_prior)), b(zero_vec4()) {}
...
struct EBTS {
    ...
    EBTS(int n_, double lambda_prior, uint64_t seed)
        : n(n_),
          rng(seed),
          Lambda_local(n_, identity_mat4(lambda_prior)),
...
std::vector<double> run_algorithm_mean_regret_over_time(SimpleLinearMDP &env, Alg &alg, int T) {
    ...
    for (int t = 0; t < T; ++t) {
        ...
        mean_regret_over_time[t] = mean_regret / static_cast<double>(env.n);
...
std::vector<double> average_runs(const std::string &algo_name, const Scenario &sc) {
    std::vector<double> avg(sc.T, 0.0);

    for (int run = 0; run < sc.n_runs; ++run) {
        uint64_t env_seed = 123456 + run;
        uint64_t alg_seed = 987654 + run;

        SimpleLinearMDP env(sc.n, sc.mu_theta, sc.Sigma_theta, sc.alpha, sc.beta, sc.gamma, sc.sigma_s, sc.sigma_r,
                            sc.mu_s0, sc.sigma_s0, env_seed);

        std::vector<double> one_run;
        if (algo_name == "unpooled_ts") {
            UnpooledTS alg(sc.n, sc.lambda_prior, alg_seed);
            one_run = run_algorithm_mean_regret_over_time(env, alg, sc.T);
        } else if (algo_name == "pooled_ts") {
            PooledTS alg(sc.n, sc.lambda_prior, alg_seed);
            one_run = run_algorithm_mean_regret_over_time(env, alg, sc.T);
        } else if (algo_name == "eb_ts") {
            EBTS alg(sc.n, sc.lambda_prior, alg_seed);
            one_run = run_algorithm_mean_regret_over_time(env, alg, sc.T);
        } else {
            throw std::runtime_error("Unknown algorithm name");
        }

        for (int t = 0; t < sc.T; ++t) avg[t] += one_run[t];
    }

    for (int t = 0; t < sc.T; ++t) avg[t] /= static_cast<double>(sc.n_runs);
    return avg;
}

void append_scenario_csv(std::ofstream &out, const Scenario &sc, const std::vector<double> &unpooled,
                         const std::vector<double> &pooled, const std::vector<double> &eb) {
    for (int t = 0; t < sc.T; ++t) {
        out << sc.name << "," << (t + 1) << "," << unpooled[t] << "," << pooled[t] << "," << eb[t] << "\n";
    }
}

int main() {
    Mat4 sigma_original = zero_mat4();
    sigma_original[0][0] = 0.5;
    sigma_original[1][1] = 0.5;
    sigma_original[2][2] = 0.25;
    sigma_original[3][3] = 0.25;

    Mat4 sigma_low = zero_mat4();
    sigma_low[0][0] = 0.02;
    sigma_low[1][1] = 0.02;
    sigma_low[2][2] = 0.02;
    sigma_low[3][3] = 0.02;

    Mat4 sigma_high = zero_mat4();
    sigma_high[0][0] = 1.0;
    sigma_high[1][1] = 1.0;
    sigma_high[2][2] = 0.8;
    sigma_high[3][3] = 0.8;

    Mat4 sigma_mid = zero_mat4();
    sigma_mid[0][0] = 0.15;
    sigma_mid[1][1] = 0.15;
    sigma_mid[2][2] = 0.15;
    sigma_mid[3][3] = 0.15;

    Vec4 mu = {0.0, 1.0, 0.5, -0.25};

    std::vector<Scenario> scenarios = {
        {"original", 50, 100, 500, 1.0, mu, sigma_original, 0.0, 0.8, 0.4, 0.5, 1.0, 0.0, 1.0},
        {"low_hetero_short_noisy", 50, 40, 500, 1.0, mu, sigma_low, 0.0, 0.8, 0.4, 0.5, 1.5, 0.0, 1.0},
        {"high_hetero_long_low_noise", 50, 200, 500, 1.0, mu, sigma_high, 0.0, 0.8, 0.4, 0.5, 0.5, 0.0, 1.0},
        {"mid_hetero_very_short_high_noise", 200, 20, 500, 1.0, mu, sigma_mid, 0.0, 0.8, 0.4, 0.5, 2.0, 0.0, 1.0},
    };

    const std::string out_file = "multi_scenario_mean_regret_500_runs.csv";
    std::ofstream out(out_file);
    out << std::fixed << std::setprecision(8);
    out << "scenario,time,unpooled_ts_mean_regret,pooled_ts_mean_regret,eb_ts_mean_regret\n";

    for (const auto &sc : scenarios) {
        std::cout << "Running scenario: " << sc.name << "...\n";
        auto unpooled = average_runs("unpooled_ts", sc);
        auto pooled = average_runs("pooled_ts", sc);
        auto eb = average_runs("eb_ts", sc);
        append_scenario_csv(out, sc, unpooled, pooled, eb);
    }

    out.close();
    std::cout << "Done. Wrote " << out_file << "\n";
    return 0;
}
