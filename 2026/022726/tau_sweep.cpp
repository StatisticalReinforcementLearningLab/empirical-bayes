...
int main(){
    ...
    double sigma = 1.0; // within-person noise
    int runs = 500;
    
    Vec mu0 = {0.0, 0.0};
    Vec tau0 = {1.0, 1.0};
    
    // Build custom tau values: 0.01-0.1 by 0.01, 0.1-1 by 0.1, 1-10 by 0.5
    Vec tau_values;
    
    // 0.01 to 0.1 by 0.01
    for (double tau = 0.01; tau <= 0.1 + 1e-9; tau += 0.01) {
        tau_values.push_back(tau);
    }
    
    // 0.2 to 1.0 by 0.1 (0.1 already added)
    for (double tau = 0.2; tau <= 1.0 + 1e-9; tau += 0.1) {
        tau_values.push_back(tau);
    }
    
    // 1.5 to 10.0 by 0.5 (1.0 already added)
    for (double tau = 1.5; tau <= 10.0 + 1e-9; tau += 0.5) {
        tau_values.push_back(tau);
    }
    
    int n_tau = tau_values.size();
    
    std::ofstream out("tau_sweep_results.csv");
    out << "tau,mean_unpooled,se_unpooled,mean_pooled,se_pooled,mean_eb,se_eb,winner\n";
    
    std::cout << "Sweeping " << n_tau << " tau values from " << tau_values[0] 
              << " to " << tau_values[n_tau-1] << "...\n";
    
    for (int tau_idx = 0; tau_idx < n_tau; ++tau_idx) {
        double tau = tau_values[tau_idx];
        ...
        for (int r = 0; r < runs; ++r) {
            std::mt19937_64 rng(r + 10000 + tau_idx * 1000);
            Vec mu_a = {0.0, mu2};
            Vec tau_a = {tau, tau};
        ...
        if ((tau_idx + 1) % 5 == 0) {
            std::cout << "Progress: " << (tau_idx + 1) << "/" << n_tau 
                     << " (tau=" << std::fixed << std::setprecision(2) << tau << ")\n";
        }
    }
    
    out.close();
    std::cout << "\nDone! Results written to tau_sweep_results.csv\n";
    std::cout << "Summary: Check which algorithm wins in different tau regimes.\n";
