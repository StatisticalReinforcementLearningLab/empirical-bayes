        ...
        for (int i=0;i<n;++i) {
            ...
            if (!has_data[i][0] || !has_data[i][1]) {
                ...
                for (int a=0;a<2;++a) {
                    ...
                    // }
                    if (t == T-1) {
                        M3 Si_inv3 = D_Si_inv_Dt(c, Si_inv);          // individual precision lifted to 3D
                        double tr_pop = Sp_inv[a].m[0][0] + Sp_inv[a].m[1][1] + Sp_inv[a].m[2][2];
...
int main() {
    ...
    std::ofstream out("testing_code/conj_1_small_n.csv");
    ...
    out.close();
    std::cout << "\nDone! Results written to testing_code/conj_1_small_n.csv\n";
    return 0;
}
