...
using Cube = std::vector<Mat>;   // [context][unit][arm]
using ICube = std::vector<IMat>; // [context][unit][arm]
...
inline double mean_reward(const Env& env, int i, int a, int ctx) {
    return env.theta0[i][a] + env.theta1[i][a] * static_cast<double>(ctx);
}

Vec run_thompson_context_aware(
    int T,
    int n,
    int arms,
    const Env& env,
    ...
    ICube N(2, IMat(n, IVec(arms, 0)));
    Cube S(2, Mat(n, Vec(arms, 0.0)));
    ...
    for (int t = 0; t < T; ++t) {
        for (int i = 0; i < n; ++i) {
            ...
            for (int a = 0; a < arms; ++a) {
                double sig2 = env.sigma_a[a] * env.sigma_a[a];
                double prec_prior = 1.0 / (tau0[a] * tau0[a]);
                double prec_data = static_cast<double>(N[ctx][i][a]) / sig2;
                double post_var = 1.0 / (prec_prior + prec_data);
                double post_mean = post_var * (mu0[a] / (tau0[a] * tau0[a]) + S[ctx][i][a] / sig2);
                if (N[ctx][i][a] == 0) {
                    post_mean = mu0[a];
            ...
            const double reward_mean = mean_reward(env, i, chosen, ctx);
            std::normal_distribution<double> rd(reward_mean, env.sigma_a[chosen]);
            double reward = rd(rng);
            N[ctx][i][chosen] += 1;
            S[ctx][i][chosen] += reward;

            double best_theta = -INFINITY;
            for (int a = 0; a < arms; ++a) {
                best_theta = std::max(best_theta, mean_reward(env, i, a, ctx));
            }
            double inst_regret = best_theta - reward_mean;
...
Vec run_pooled_thompson_context_aware(
    int T,
    int n,
    int arms,
    const Env& env,
    ...
    IMat N(2, IVec(arms, 0));
    Mat S(2, Vec(arms, 0.0));
    Vec cum_regret_sum(T, 0.0);
    std::bernoulli_distribution bd(p_one);
    ...
    for (int t = 0; t < T; ++t) {
        for (int i = 0; i < n; ++i) {
            ...
            for (int a = 0; a < arms; ++a) {
                double sig2 = env.sigma_a[a] * env.sigma_a[a];
                double prec_prior = 1.0 / (tau0[a] * tau0[a]);
                double prec_data = static_cast<double>(N[ctx][a]) / sig2;
                double post_var = 1.0 / (prec_prior + prec_data);
                double post_mean = post_var * (mu0[a] / (tau0[a] * tau0[a]) + S[ctx][a] / sig2);
                if (N[ctx][a] == 0) {
                    post_mean = mu0[a];
                    post_var = tau0[a] * tau0[a];
                }
...
Vec run_empirical_bayes_context_aware(
    int T,
    int n,
    int arms,
    const Env& env,
    ...
    Cube m(2, Mat(n, Vec(arms, 0.0)));
    Cube v(2, Mat(n, Vec(arms, 0.0)));
    for (int c = 0; c < 2; ++c) {
        for (int i = 0; i < n; ++i) {
            for (int a = 0; a < arms; ++a) {
                m[c][i][a] = mu0[a];
                v[c][i][a] = tau0[a] * tau0[a];
            }
        }
    }
    ...
    for (int t = 0; t < T; ++t) {
        Mat mu_hat(2, Vec(arms, 0.0));
        Mat tau2_hat(2, Vec(arms, 0.0));
        ...
        for (int c = 0; c < 2; ++c) {
            for (int a = 0; a < arms; ++a) {
                ...
                double var_m = 0.0;
                for (int i = 0; i < n; ++i) {
                    double d = m[c][i][a] - mean_m;
                    var_m += d * d;
                }
                var_m = (n > 1) ? var_m / static_cast<double>(n - 1) : 0.0;

                mu_hat[c][a] = mean_m;
                tau2_hat[c][a] = std::max(0.0, var_m - mean_v);
            }
        }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int a = 0; a < arms; ++a) {
                double lam = (tau2_hat[ctx][a] > 0.0)
                    ? (tau2_hat[ctx][a] / (tau2_hat[ctx][a] + v[ctx][i][a]))
                    : 1.0;
                m_eb[a] = lam * m[ctx][i][a] + (1.0 - lam) * mu_hat[ctx][a];
                v_eb[a] = (tau2_hat[ctx][a] > 0.0)
                    ? (v[ctx][i][a] * tau2_hat[ctx][a]) / (v[ctx][i][a] + tau2_hat[ctx][a])
                    : 0.0;
            }
            ...
            double sig2 = env.sigma_a[chosen] * env.sigma_a[chosen];
            double prec = 1.0 / v[ctx][i][chosen] + 1.0 / sig2;
            double v_new = 1.0 / prec;
            double m_new = v_new * (m[ctx][i][chosen] / v[ctx][i][chosen] + reward / sig2);
            m[ctx][i][chosen] = m_new;
            v[ctx][i][chosen] = v_new;
...
int main(int argc, char* argv[]) {
    // Keep defaults aligned with final_regret_vs_distance_ratio.cpp.
    const int runs = 500;
    const int n = 100;
    const int arms = 2;
    const int T = 200;
    ...
    double p_one = 0.2;
    std::string out_csv = "final_regret_vs_distance_ratio_context_aware.csv";
    ...
    for (double ratio : ratios) {
        ...
        for (int run = 0; run < runs; ++run) {
            ...
            Env env = set_environment_contextual(
                n, arms, mu_a, tau_a, theta_ctx_mu, theta_ctx_tau, sigma_a, rng_env
            );

            Vec ts = run_thompson_context_aware(T, n, arms, env, p_one, mu0, tau0, rng_ts);
            Vec pts = run_pooled_thompson_context_aware(T, n, arms, env, p_one, mu0, tau0, rng_pts);
            Vec eb = run_empirical_bayes_context_aware(T, n, arms, env, p_one, mu0, tau0, rng_eb);
